...
def main() -> None:
    ...
    # 0.1.5 was built by appending the saved-repository patch to 0.1.4.
    # Rebuild from the same clean 0.1.4 base so 0.1.6 contains exactly one
    # copy of the saved-repository implementation, now including repo notes.
    ...
    manifest["description"] = (
        "Plan mentorship studies, pull released Units, restore saved student work "
        "and private Markdown notes from GitHub, memorize passage summaries, and "
        "keep meeting notes."
    )
    ...
    for name in ["main.js", "manifest.json", "styles.css", "versions.json"]:
        ...
        release_files.append(
            {
                "name": name,
                "path": f"downloads/plugin/{VERSION}/{name}",
                "sha256": sha256(path),
            }
        )
